...
- Any alternate top-level contract structure must fail Stage 1 validation.
...
- No additional required tenant fields may be introduced outside schema versioning
- Minimal workload remains valid with only required `spec` keys
...
- Missing required canonical key (`apiVersion`, `kind`, `metadata`, `spec`)
...
- Unsupported but declared delivery strategy is rejected
...
# Delivery Capability Matrix (v0.1)

This matrix is normative for validation outcomes.
...
| `spec.build.mode` | `managed` | Reserved | Rejected with compatibility error |

Unsupported strategies or modes must fail validation before merge.
