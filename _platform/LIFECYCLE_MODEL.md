...
- Lifecycle transitions are derived from
  `CONTRACT_SCHEMA.md` → `Canonical Workload Contract Definition`.
- Contract version changes must pass migration and re-validation before generation.
...
- Generator version stamped in generated metadata
...
- Generator version in manifests matches the allowed generator version policy
...
- Reconciliation halts on generator-version drift until artifacts are regenerated
...
Delivery strategy mapping in Formation (`v0.1`):

- `rolling`: supported for promotions
- `recreate`, `blue-green`, `canary`: reserved; promotion must be rejected at validation
...
Contract version transition behavior:
...
```
Draft (version change) → Validated (migration) → Generated → Updated
```
